    ...
    public class TCPServer
    {
        ...
        public string StartServer(IPEndPoint ip)
        {
            ...
            try
            {
                ...
                if (ShowListMsg != null)
                    ShowListMsg(null, string.Format("服务器【{0}】启动成功...", SocketServer.LocalEndPoint), DateTimeHelper.Now);
                Running = true;
            ...
            {
                if (ShowListMsg != null)
                    ShowListMsg(null, string.Format("服务器【{0}】启动失败", ip), DateTimeHelper.Now);
                LogHelper.WriteLog(string.Format("服务器【{0}】启动失败", ip), ex);
            }
            return null;
        }
        ...
        public string StartServer(string ip, int port)
        {
            ...
            {
                if (ShowListMsg != null)
                    ShowListMsg(null, string.Format("服务器【{0}:{1}】启动失败", ip, port), DateTimeHelper.Now);
                LogHelper.WriteLog(string.Format("服务器【{0}:{1}】启动失败", ip, port), ex);
            }
            return null;
        }
        /// <summary>
        ...
        public void StopServer()
        {
            try
            {
                ...
                if (ShowListMsg != null)
                    ShowListMsg(null, "服务器停止", DateTimeHelper.Now);
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine(ex);
                if (ShowListMsg != null)
                    ShowListMsg(null, "服务器停止异常", DateTimeHelper.Now);
        ...
        private void AcceptCallBack(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    sokclient = null;
                    if (ShowListMsg != null)
                        ShowListMsg(null, key + "重复请求已阻止", DateTimeHelper.Now);
                }
                catch (Exception)
                {
                    ...
                    Clients.Add(client);
                    if (ShowListMsg != null)
                        ShowListMsg(client, "建立连接", DateTimeHelper.Now);
                }
            }
            catch (Exception ex)
            {
                ...
                {
                    if (ShowListMsg != null)
                        ShowListMsg(null, "处理客户端连接请求失败", DateTimeHelper.Now);
            ...
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine(ex);
                if (ShowListMsg != null)
                    ShowListMsg(null, "服务器启动侦听失败", DateTimeHelper.Now);
